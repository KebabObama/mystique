  ...
  socket.on("friend:send", async ({ senderId, receiverId }) => {
    try {
      await db
        ...
        .values({ id: randomUUID(), sender: senderId, receiver: receiverId });
  ...
  socket.on("friend:accept", async ({ senderId, receiverId }) => {
    try {
      ...
      await db
        ...
        .values({ id: randomUUID(), a: senderId, b: receiverId });
      ...
      await db
        ...
        .where(
          or(
            ...
            and(eq(schema.friend.a, blockedUserId), eq(schema.friend.b, userId))
          )
        );
  ...
  socket.on("group:create", async ({ creatorId, userIds, name }) => {
    try {
      ...
      const [newGroup] = await db
        .insert(schema.group)
        .values({ id: randomUUID(), users, name })
