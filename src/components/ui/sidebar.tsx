...
const Sidebar = ({
  ...
  if (isMobile) {
    return (
      <Sheet onOpenChange={setOpenMobile} open={openMobile} {...props}>
        <SheetContent
          className="bg-card text-sidebar-foreground mt-4 -ml-1.5 h-[calc(100dvh-2rem)] w-(--sidebar-width) p-0 pl-3 md:mt-0 md:ml-0 md:pl-0 [&>button]:hidden"
          data-mobile="true"
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="bg-card group-data-[variant=floating]:border-sidebar-border flex h-full w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow-sm"
          data-sidebar="sidebar"
...
const SidebarRail = ({ className, ...props }: React.ComponentProps<"button">) => {
  ...
  return (
    <button
      ...
      className={cn(
        "hover:after:bg-card-border absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 sm:flex",
        "in-data-[side=left]:cursor-w-resize in-data-[side=right]:cursor-e-resize",
        "[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize",
        "hover:group-data-[collapsible=offcanvas]:bg-card group-data-[collapsible=offcanvas]:translate-x-0 group-data-[collapsible=offcanvas]:after:left-full",
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button duration-200 active:scale-96 flex w-full transition-all items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-hidden ring-ring transition-[width,height,padding,transform] focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-muted data-[active=true]:font-medium data-[state=open]:hover:bg-muted group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        outline:
          "bg-transparent shadow-[0_0_0_1px_hsl(var(--border))] hover:shadow-[0_0_0_1px_hsl(var(--ring))] text-foreground",
      },
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs",
