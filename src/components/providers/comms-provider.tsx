...
export const CommunicationProvider = ({
  ...
  const sendFriendRequest = useCallback(
    (receiverId: string) => {
      if (!userId) return;
      socketRef.current?.emit("friend:send", { senderId: userId, receiverId });
    },
    [userId]
  );
  ...
  const denyRequest = useCallback(
    (senderId: string) => {
      if (!userId) return;
      socketRef.current?.emit("friend:deny", { senderId, receiverId: userId });
    },
    [userId]
  );
  ...
  const removeFriend = useCallback(
    (friendId: string) => {
      if (!userId) return;
      socketRef.current?.emit("friend:remove", { userId, friendId });
    },
    [userId]
  );
  ...
  const blockUser = useCallback(
    (blockedUserId: string) => {
      if (!userId) return;
      socketRef.current?.emit("user:block", { userId, blockedUserId });
    },
    [userId]
  );
