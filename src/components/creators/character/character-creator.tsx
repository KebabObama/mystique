  ...
  return (
    ...
    >
      ...
      <Dialog.Content className="min-w-md md:min-w-3xl lg:min-w-6xl">
        ...
        <div className="h-full max-h-120 overflow-auto p-2">{render()}</div>
        ...
        {RACES.map((e, i) => {
          ...
          return (
            <button
              ...
              className={`group border-border/20 relative aspect-9/16 basis-full overflow-visible border-2 p-2 transition-all duration-300 ${isSelected ? "" : "hover:border-border grayscale-100 hover:scale-105 hover:grayscale-50"}`}
            >
              {isSelected && <Border />}
              <Image
                src={`/classes/${e}.png`}
...
const _Attributes = ({ points = 18 }: { points?: number }) => {
  ...
  return (
    <section className="flex flex-col gap-9 p-4 text-center text-xl">
      Allocate all points: ({current}/{points})
      <div className="grid grid-cols-1 gap-x-9 gap-y-3 md:grid-cols-2 xl:grid-cols-3">
        {ATTRIBUTES.map((e) => {
          const attr = store.character.attributes[e];
          return (
            <div
              key={e}
              className="flex flex-row items-center justify-between text-lg capitalize"
            >
              {e}
