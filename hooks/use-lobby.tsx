...
export type LobbyStore = {
  ...
  disconnect: () => void;
  sendMessage: (userId: string, lobbyId: string, content: string) => void;
  joinLobby: (userId: string, lobbyId: string) => void;
  createLobby: (userId: string, name: string) => void;
...
export const useLobby = create<LobbyStore>((set, get) => ({
  ...
  createLobby: (userId, name) => get().socket?.emit("lobby:create", userId, name),
  joinLobby: (userId, lobbyId) => get().socket?.emit("lobby:join", userId, lobbyId),
  sendMessage: (userId, lobbyId, content) =>
    get().socket?.emit("lobby:send", userId, lobbyId, content),
}));

export const LobbyProvider = ({ children }: Readonly<{ children: React.ReactNode }>) => {
